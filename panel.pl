...
my ($window, @workspaces, $label, $clock);
...
## movement should be handled in WM
...
# Set workspace color
sub ws_set_color($ws, $new_color_bg, $new_color_fg) {
    my $ws = $workspaces[$ws - 1];
...
sub ws_set_active($new_active) {
    for my $ws (@workspaces) {
        if ($ws->{active}) {
            ...
            $ws->{active} = undef;
            ws_set_color($ws->{id}, $color_bg, $color_fg);
        }

        if ($ws->{id} == $new_active) {
            $ws->{active} = 1;
            $ws->{urgent} = undef;
            ws_set_color($new_active, $color_fg, $color_bg);
        }
    }
}

# Set workspace urgency
sub ws_set_urgent($ws_id, $urgent = 1) {
    my $ws = $workspaces[$ws_id - 1];
    return if $ws->{active};
    $ws->{urgent} = $urgent ? 1 : undef;
    ws_set_color($ws_id, $urgent ? ($color_urgent_bg, $color_urgent_fg) : ($color_bg, $color_fg));
}
...
sub ws_create($title = "") {
    ...
    $ebox->signal_connect('button-press-event', sub ($obj, $e) { ws_set_active $my_id if $e->button == 1 });
    $ebox->add($label);
...
# Fill in @workspaces
@workspaces = map { ws_create($_) } @ws_names;
ws_set_active(1);
...
$window->add($hdbar);
$window->show_all;
ws_set_visible($_, 0) for 1..@ws_names;
